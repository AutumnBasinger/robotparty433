...
void ws2812b_setup() {
    ...
    TRISBbits.TRISB6 = 0; // made TRISB an output
    LATBbits.LATB6 = 0; // turned TRISB off
}

// build an array of high/low times from the color input array, then output the high/low bits
void ws2812b_setColor(wsColor * c, int numLEDs) { // 24 bits each LED, every bit is a high and low
    int i = 0; int j = 0; // for loops, 8 bits for color, 3 colors each
    ...
    for (i = 0; i < numLEDs; i++) { // for every WS2812B LED (i), loop through colors
        // red
        for (j = 7; j >= 0; j--) { // loop through each color bit (j), MSB first
            if ((c[i].r >> j) & 0b1 == 0b1) { // if bit in c[].r is 1
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME; // high longer
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME; // low shorter
                nB++;
            } 
            else { // if bit in c[]r is 0
                delay_times[nB] = delay_times[nB - 1] + LOWTIME; // low longer
                nB++;
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME; // high shorter
                nB++;
            }
        }
        // green
        for (j = 7; j >= 0; j--) { // loop through each color bit (j), MSB first
            if ((c[i].g >> j) & 0b1 == 0b1) { // if bit in c[].r is 1
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME; // high longer
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME; // low shorter
                nB++;
        ...
        // blue
        for (j = 7; j >= 0; j--) { // loop through each color bit (j), MSB first
            if ((c[i].b >> j) & 0b1 == 0b1) { // if bit in c[].r is 1
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME; // high longer
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME; // low shorter
                nB++;
