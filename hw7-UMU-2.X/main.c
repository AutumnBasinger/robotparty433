...
//    RPB3Rbits.RPB3R = 0b0001; // Set B3 to U1TX
    ...
    while (1) {
        ...
        for (i=0; i<NUM_DATA_PNTS; i++){
            _CP0_SET_COUNT(0);
            // read IMU
            burst_read_mpu6050(IMU_buf); // get data from chip
            ax[i] = conv_xXL(IMU_buf); // turns data into 16b and their floating point numbers
            ...
            // gyro data
            if (i>0) {
                thxg[i] = thxg[i-1] + gx[i] * 0.01;
                thyg[i] = thyg[i-1] + gy[i] * 0.01;
            }
            
            // accel data
            thxa[i] = atan(az[i]/ax[i]);
            thya[i] = atan(az[i]/ay[i]);
            
            // average
            avx[i] = (thxg[i]+thxa[i])/2;
            avy[i] = (thyg[i]+thya[i])/2;
        ...
        // print data
        // ls /dev/tty.*
        // screen /dev/tty.usbserial-0232C13E 230400 [for UART]
        for (i=0; i<NUM_DATA_PNTS; i++){
            sprintf(m_out,"%d %f %f %f %f %f %f %f %f %f %f %f %f %f\r\n",NUM_DATA_PNTS-i,ax[i],ay[i],az[i],gx[i],gy[i],gz[i],temp[i],thxg[i],thyg[i],thxa[i],thya[i],avx[i],avy[i]);
            WriteUART1(m_out);
        }
